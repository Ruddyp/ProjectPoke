...
} from "@/models/pokebattle_leaderboard";
...
export async function getPokeBattleLeaderboard() {
  try {
    await connectDB();
    const data: IPokeBatlle[] = await PokeBattleLeaderboard.find()
      .sort({
        score: 1,
      })
      .lean();
...
export async function addScoreToPokeBattleLeaderboard(
  scoreData: Partial<IPokeBatlle>,
): Promise<IPokeBatlle> {
  try {
    await connectDB();

    const newEntry = await PokeBattleLeaderboard.create(scoreData);
    ...
    revalidatePath("/games/PokeBattle"); // Chemin mis à jour
    return result;
  } catch (error) {
    console.error("Erreur lors de l'ajout du score:", error);
    throw new Error("Impossible de mettre à jour le score.");
...
export async function replaceScoreToPokeBattleLeaderboard(
  idToUpdate: string,
  scoreToPush: Partial<IPokeBatlle>,
): Promise<IPokeBatlle> {
  try {
    await connectDB();
    ...
    // { new: true } permet de récupérer le document APRES la modification
    // .lean() pour renvoyer un objet JS
    const updatedScore = await PokeBattleLeaderboard.findByIdAndUpdate(
      idToUpdate,
      { $set: scoreToPush },
      { new: true, runValidators: true },
    ).lean();
