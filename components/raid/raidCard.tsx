        ...
        <Card className="m-0.5 rounded-md border border-slate-500 w-[315px]">
            ...
            <CardContent className="flex flex-col justify-center items-center p-1">
                <div className="flex flex-row justify-center items-center gap-2 p-1.5 border-b border-slate-500 w-full">
                    <p className="text-base sm:text-lg font-bold text-center text-red-400">{pokemon.name.fr}</p>
                    <PokemonTypesComponent pokemon={pokemon} />
                </div>
                <div onClick={() => router.push(`/pokemon/${raid.pokemon_id}`)} className="relative size-48 xs:size-56 sm:size-64 cursor-pointer">
                    <Image
                    ...
                    <table className="w-full table-fixed">
                        ...
                        <tbody>
                            <tr className="border-b border-slate-500">
                                <td className="text-red-400 p-2 text-right font-medium">PC du boss :</td>
                                <td className="p-2">{raid.boss_cp != undefined ? <span className="font-bold">{raid.boss_cp}</span> : "Inconnu"}</td>
                            </tr>
                            <tr className="border-b border-slate-500">
                                <td className="text-red-400 p-2 text-right font-medium">PC max :</td>
                                <td className="p-2">{raid.max_cp != undefined ? <span className="font-bold">{raid.max_cp}</span> : "Inconnu"}</td>
                            </tr>
                            <tr className={`${raid.additional_info != undefined ? "border-b border-slate-500" : ""} `}>
                                <td className="text-red-400 p-2 text-right font-medium">PC max boost météo :</td>
                                <td className="p-2">{raid.max_cp_boost != undefined ? <span className="font-bold">{raid.max_cp_boost}</span> : "Inconnu"}</td>
                            </tr>
                            {raid.additional_info != undefined ?
                                <tr>
                                    <td className="text-red-400 p-2 text-right font-medium">Information :</td>
                                    <td className="p-2 font-bold text-sm">{raid.additional_info}</td>
                                </tr>
                                : null
                            }
                        </tbody>
