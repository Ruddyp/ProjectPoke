...
function getFilteredPokemonBySearch(pokemons: Pokemon[], searchValue: string) {
  ...
  return pokemons.filter((pokemon) =>
    pokemon.name.fr.toLowerCase().includes(searchValue),
  );
}
...
export default function PokemonCombobox() {
  ...
  return (
    <Command className="w-[230px] bg-background">
      ...
      />
      <CommandList
        className={`${searchValue == "" ? "hidden" : ""} bg-secondary-foreground rounded-md mx-1 border border-slate-500`}
      >
        <CommandEmpty>Aucun pokémon trouvé.</CommandEmpty>
        <CommandGroup>
          {searchValue != "" &&
            getFilteredPokemonBySearch(pokemons, searchValue).map((pokemon) => (
              <CommandItem
                ...
                onSelect={(currentValue) => {
                  if (tab == "pokemon_go") {
                    router.push(
                      `/pokemon/${pokemon.pokedex_id}?tab=${tab}&tab_go=${tabGo}`,
                    );
                  } else {
