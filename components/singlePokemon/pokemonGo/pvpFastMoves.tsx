...
function RowFastMove({ pokemonGoPvpFastMoves, movesInfo, move, types, isElite }: RowFastMoveProps) {
    const moveInfo = pokemonGoPvpFastMoves.find((fastMove) => fastMove.name == move);
...
export default function PvpFastMoves({ pokemon, pokemonGoPvpFastMoves, pokemonMoves, movesInfo, types }: PvpFastMovesProps) {
    ...
    return (
        <table className="w-full table-auto">
            ...
            <tbody>
                {pokemonMoves.fast_moves.map((move, index) => {
                    let border = "border-b border-slate-500"
                    if (pokemonMoves.fast_moves.length - 1 == index && pokemonMoves.elite_fast_moves.length == 0) {
                        border = ""
                    }
                    const typeInfo = types.find(type => type.name.en.toLowerCase() == movesInfo[move]?.type.name.toLowerCase())
                ...
                {pokemonMoves.elite_fast_moves.map((move, index) => {
                    let border = "border-b border-slate-500"
                    if (pokemonMoves.elite_fast_moves.length - 1 == index) {
                        border = ""
                    }
                    const typeInfo = types.find(type => type.name.en.toLowerCase() == movesInfo[move]?.type.name.toLowerCase())
                    const bgColor = colors[typeInfo?.name.fr.toLowerCase() as PokemonColorType];
