      ...
      $(container)
        ...
        .each((_, element) => {
          ...
          if (elem.hasClass("pogo-list-header")) {
            ...
            elem.hasClass("pogo-list-item") ||
            elem.hasClass("pogo-list-item-desc")
          ) {
            // Récupérer l'ID du Pokémon
            const pokemonId = cleanNumber(
              elem.find(".pogo-list-item-number").text().replace("#", ""),
            );
            ...
            const regex =
              /Boss CP\s*([\d,]+)Max capture CP\s*([\d,]+)\s*([\d,]+)/;
