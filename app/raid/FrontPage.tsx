...
import RaidFilter, {
  raidFilterInitValue,
  RaidFilterType,
} from "@/components/raid/raidFilter";
...
function getFilteredRaidBySearch(
  pokemons: Pokemon[],
  raids: Raid[],
  searchValue: string
) {
  ...
  return raids.filter((raid) => {
    const pokemonInfo = pokemons.find(
      (pokemon) => pokemon.pokedex_id == raid.pokemon_id
    );
...
export default function FrontPage({ raids, pokemons }: FrontPageProps) {
  ...
  if (raids == undefined)
    return <p>Erreur lors de la récupération de la liste des raids</p>;

  const filteredRaids = getFilteredRaid(
    pokemons,
    raids,
    searchValue,
    raidFilter
      ...
      <div className="flex flex-row flex-wrap items-start justify-center w-full gap-6">
        {filteredRaids.map((raid: Raid, index: number) => {
          const pokemon = pokemons.find(
            (poke) => poke.pokedex_id == raid.pokemon_id
          );
          if (pokemon == undefined)
            return (
              <p key={raid.pokemon_id}>
                Erreur lors de la récupération des information du pokemon
              </p>
            );
          return (
